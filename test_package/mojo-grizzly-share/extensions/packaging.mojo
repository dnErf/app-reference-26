...
fn package_init(name: String, version: String):
    ...
    let file = open(String(toml_path), "w")
    file.write("[tool.hatch.build]\npackages = [\".\"]\n")
    file.close()
...
fn add_python_dep(dep: String):
    ...
    let file = open(String(toml_path), "a")
    file.write("dependencies = [\"" + dep + "\"]\n")
    file.close()
...
fn package_build():
    ...
    for file in current_package.mojo_files:
        py_subprocess.run(["mojo", "build", file], cwd=current_package.name)
    ...
    let file = open(current_package.name + "/dist/run.sh", "w")
...
fn pixi_init():
    ...
    let file = open(String(pixi_path), "w")
...
fn pixi_add_dep(dep: String):
    ...
    let py_path = Python.import_module("pathlib").Path
    let pixi_path = py_path(current_package.name) / "pixi.toml"
    let file = open(String(pixi_path), "a")
    file.write("dependencies = [\"" + dep + "\"]\n")
    file.close()
...
fn hatch_init():
    ...
    let file = open(String(hatch_path), "w")
